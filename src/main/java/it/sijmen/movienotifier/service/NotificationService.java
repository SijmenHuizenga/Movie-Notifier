...
public class NotificationService {
  ...
  public NotificationService(
      ...
      @Value("${mailgun.from.mail}") String fromMail)
      throws IOException {
    this.userRepository = userRepository;
    mailgunConfig = new Configuration().domain(domain).apiKey(apikey).from(fromName, fromMail);

    if (!"disabled".equals(serviceAccountKeyFile))
      FirebaseApp.initializeApp(
          new FirebaseOptions.Builder()
              .setCredentials(
                  GoogleCredentials.fromStream(new FileInputStream(serviceAccountKeyFile)))
              .build());
  }
  ...
  public void sendUpdate(
    ...
    if (mailError != null && firebaseError != null) {
      throw new IOException(
          "Notification to mail and firebase failed: \n"
  ...
  private void sendAndroidNotification(
      User user, String body, String watcherId, String watcherName, int matchCount, int movieid)
      throws IOException {
    AndroidConfig config = AndroidConfig.builder().setPriority(AndroidConfig.Priority.HIGH).build();

    List<Message> messages =
        user.getRegistrationTokens().stream()
            .map(
                token ->
                    Message.builder()
                        .setToken(token)
    ...
    try {
      ...
      if (response == null) throw new IOException("Firebase returned null.");
    ...
    if (response.getFailureCount() > 0) {
      ...
      String error =
          String.format(
              "%d of the %d messages failed: \n%s",
              response.getFailureCount(), messages.size(), failureMessage.toString());
  ...
  private void sendEmail(String email, String messageHeader, String messageBody)
    ...
    if (!response.isOk()) {
      LOGGER.error(
          "Mailgun returned not ok. Code: {}. Message: {}",
          response.responseCode(),
          response.responseMessage());
      throw new IOException(
          "Mailgun returned not ok. code: "
              + response.responseCode()
              + ", Message: "
              + response.responseMessage());
    }
    ...
    LOGGER.trace(
        "Sent mail message through mailgun to {}. Message: {} {}",
        email,
        messageHeader,
        messageBody);
  }
