...
class WatcherIT {
  ...
  void read(String id) {
    checkWatcherResponse(
        given().header(HEADKEY, user.apikey()).when().get("/watchers/" + id).then());
  ...
  private void makeRandomFilters(boolean includeNulls) {
    Map<String, Object> out =
        (Map<String, Object>) watcherdata.getOrDefault(FILTERS, new HashMap<>());
    out.put("cinemaid", "PATHE" + r.nextInt(18));
    out.put("startafter", r.nextInt(500) + 500);
    out.put("startbefore", r.nextInt(500) + 1001L);
    String[] opts =
        new String[] {
          "ov", "nl", "imax", "hfr", "laser", "dbox", "dolbycinema", "dolbyatmos", "3d", "4k", "4dx"
        };
    for (String o : opts) {
  ...
  String id() {
    return (String) watcherdata.get(ID);
  }
}
