...
public class WatcherCreateTest extends WatcherTestBase {
  ...
  public void testCreateSuccess() throws Exception {
    ...
    when(watcherRepo.save((Watcher) any()))
        .then(
            a -> {
    ...
    this.mvc
        .perform(
            put("/watchers/")
                ...
                .content(buildJsonCreation(testwatcher)))
    ...
    this.mvc
        .perform(
            put("/watchers/")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .header("APIKEY", testuser.getApikey())
                .content(buildJsonCreation(watcher)))
        .andExpect(status().isBadRequest())
        .andExpect(
            jsonPath("$.errors")
                .value(Matchers.containsInAnyOrder("end must be later than begin")));
    verifyZeroInteractions(watcherRepo);
  }
    ...
    this.mvc
        .perform(
            put("/watchers/")
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .header("APIKEY", testuser.getApikey())
                .content(buildJsonCreation(testuser2.getId(), testwatcher)))
        .andExpect(status().isUnauthorized());
    verifyZeroInteractions(watcherRepo);
  }
}
